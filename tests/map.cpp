...
class AVLTree {
	...
	void delete_avl(TreeNode * a) {
		TreeNode * leftsub = a->left;
		TreeNode * rightsub = a->right;

		while (leftsub)
		{
			delete_avl(leftsub);
			cout << BR << "DELETED : " << leftsub->value << NC << endl;
			delete leftsub;
			leftsub = NULL;
		}
		while (rightsub)
		{
			delete_avl(rightsub);
			cout << BR << "DELETED : " << rightsub->value << NC << endl;
			delete rightsub;
			rightsub = NULL;
		}
		cout << BR << "DELETED : " << a->value << NC << endl;
		delete a;
		a = NULL;
		return ;
	}
...
void map_tests()
{
	...
	do {
		...
		case 2:
			...
			new_node = avl.recursiveSearch(avl.root, val);
			if (new_node != NULL) {
		...
		case 5:
			...
			cout << "Height : " << avl.height(avl.root) << endl;
			break;
		case 6:
			system("cls");
			break;
